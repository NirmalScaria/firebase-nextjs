    ...
    return <div className="mx-auto max-w-md space-y-6 h-screen flex flex-col justify-center items-center transition-all p-2">
        <div className="flex flex-col shadow-lg border rounded-xl w-full bg-gray-100/40">
            <div className="flex flex-col border-b rounded-xl w-full  p-4 md:p-8 lg:p-10 py-10 items-center bg-white">
                {!currentUser ? <>
                    <h1 className="text-lg font-semibold">Sign Up With Your App</h1>
                    <h2 className="text-sm text-gray-500 mb-8 text-center">Create an account to access your awesome tool!</h2>
                    ...
                    <EmailSignUpButton email={email} password={password} setErrorMessage={setErrorMessage} setLoading={setLoading} className="w-full">
            ...
            <div className="flex flex-row justify-center text-xs font-regular text-gray-500 my-2">Existing user?
                <Link className="text-purple-500 ml-1 cursor-pointer font-medium" href={"/login"}>Sign in</Link>
            </div>
        </div>
    </div>
}
